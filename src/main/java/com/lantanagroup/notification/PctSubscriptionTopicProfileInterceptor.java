 ...
 * <p>
 * The PCT IG's GFE subscription profiles derive from that base profile but, per the IG's own published examples,
 * don't redeclare it in meta.profile - only the PCT-specific profile is listed. This interceptor adds the missing
 * base profile whenever a known PCT GFE subscription profile is present, running before HAPI's own validator
